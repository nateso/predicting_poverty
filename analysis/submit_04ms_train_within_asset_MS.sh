#!/bin/bash
#SBATCH --job-name=04ms_asset_MS_within               # Job name
...
#SBATCH --output=./results/slurm_files/slurm_04ms_within_asset_MS.out     # where to write output, %x give job name, %j names job id
#SBATCH --error=./results/slurm_files/slurm_04ms_within_asset_MS.err      # where to write slurm error
...
model_name='within_asset_MS'
cv_object_name='within_asset_MS_cv'
target_var='mean_asset_index_yeh'

data_type='LS'
id_var='delta_id'
img_folder='LS_ms_delta'
stats_file='LS_ms_delta_img_stats.pkl'

#!/bin/bash
#SBATCH --job-name=03ms_asset_MS_between               # Job name
...
#SBATCH --output=./results/slurm_files/slurm_03ms_between_asset_MS.out     # where to write output, %x give job name, %j names job id
#SBATCH --error=./results/slurm_files/slurm_03ms_between_asset_MS.err      # where to write slurm error
...
model_name='between_asset_MS'
cv_object_name='between_asset_MS_cv'
target_var='avg_mean_asset_index_yeh'

data_type='LS'
id_var='cluster_id'
img_folder='LS_median_cluster'
stats_file='LS_median_img_stats.pkl'

resnet_params='{"input_channels": 6, "use_pretrained_weights":true, "scaled_weight_init":true}'

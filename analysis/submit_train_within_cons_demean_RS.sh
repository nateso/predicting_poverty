#!/bin/bash
#SBATCH --job-name=dm_cons_RS_within               # Job name
...
#SBATCH --output=./results/slurm_files/slurm_within_demean_cons_RS.out     # where to write output, %x give job name, %j names job id
#SBATCH --error=./results/slurm_files/slurm_within_demean_cons_RS.err      # where to write slurm error
...
model_name='within_demean_cons_RS'
cv_object_name='within_demean_cons_RS_cv'

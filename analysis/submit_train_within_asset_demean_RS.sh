#!/bin/bash
#SBATCH --job-name=dm_asset_RS_within               # Job name
...
#SBATCH --output=./results/slurm_files/slurm_within_demean_asset_RS.out     # where to write output, %x give job name, %j names job id
#SBATCH --error=./results/slurm_files/slurm_within_demean_asset_RS.err      # where to write slurm error
...
model_name='within_demean_asset_RS'
cv_object_name='within_demean_asset_RS_cv'
target_var='mean_asset_index_yeh'

python -u 05_within_train_demean_RS.py "$model_name" "$cv_object_name" "$target_var"

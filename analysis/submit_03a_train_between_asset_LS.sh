#!/bin/bash
#SBATCH --job-name=03a_asset_LS_between               # Job name
...
#SBATCH --output=./results/slurm_files/slurm_03a_between_asset_LS.out     # where to write output, %x give job name, %j names job id
#SBATCH --error=./results/slurm_files/slurm_03a_between_asset_LS.err      # where to write slurm error
...
model_name='between_asset_LS'
cv_object_name='between_asset_LS_cv'
target_var='avg_mean_asset_index_yeh'

data_type='LS'
id_var='cluster_id'
img_folder='LS_rgb_median_cluster'
stats_file='LS_rgb_median_img_stats.pkl'

resnet_params='{"input_channels": 3, "use_pretrained_weights":true, "scaled_weight_init":true}'

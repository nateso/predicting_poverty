...
def plot_ls(cluster_id, fname = None, title = None):
    ...
    img_dir = f"{sat_img_dir}/satellite_imgs/{data_type}/LS_median_cluster"
...
#******************************************************************************
#........ Functions to plot other RS images ........
#******************************************************************************
...
def plot_wsf(cluster_id, fname = None, title = None):
    ...
    plt.imshow(static_img[:,:,1], cmap = 'gray')
    plt.axis('off')
    plt.title(title)
    if fname is not None:
        pth = f"../figures/sat_imgs/{fname}"
...
def plot_lc_esa(cluster_id, fname = None, title = None):
    data_type = 'RS_v2'
    sat_img_dir = "../../Data/satellite_imgs"
    img_dir = f"{sat_img_dir}/RS_v2/{data_type}_processed"
    ...
    norm = plt.cm.colors.Normalize(vmin=10, vmax=80)
    ...
    tick_values = np.linspace(10,80,7)
...
def plot_lc_modis(unique_id, fname = None, title = None):
    ...
    if fname is not None:
        pth = f"descriptives/sat_imgs/{fname}"
        plt.savefig(pth, dpi = 300, bbox_inches = 'tight')
    plt.show()

def plot_target_by_cntry(lsms_df, target_var, fname):

#!/bin/bash
#SBATCH --job-name=04a_asset_LS_within               # Job name
...
#SBATCH --output=./results/slurm_files/slurm_04a_within_asset_LS.out     # where to write output, %x give job name, %j names job id
#SBATCH --error=./results/slurm_files/slurm_04a_within_asset_LS.err      # where to write slurm error
...
model_name='within_asset_LS'
cv_object_name='within_asset_LS_cv'
target_var='mean_asset_index_yeh'

data_type='LS'
id_var='delta_id'
img_folder='LS_rgb_delta'
stats_file='LS_rgb_delta_img_stats.pkl'

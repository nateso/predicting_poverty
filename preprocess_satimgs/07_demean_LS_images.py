...
delta_rgb_img_dir = f"{sat_img_dir}/LS/LS_rgb_delta"
if not os.path.isdir(delta_rgb_img_dir):
    os.makedirs(delta_rgb_img_dir)
else:
    print(f"Warning: LS_rgb_delta Folder already exists. Files might be overwritten.")
    if input("Do You Want To Continue? [y/n]") == 'n':
        exit("Process aborted.")
...
data_type = 'LS'
mean_img_dir = f"{sat_img_dir}/{data_type}/{data_type}_rgb_median_cluster"
dynamic_img_dir = f"{sat_img_dir}/{data_type}/{data_type}_rgb"
...
for cid, uids in tqdm(cid_uid_dict.items()):
    ...
    for i, uid in enumerate(uids):
        ...
        np.save(f"{delta_rgb_img_dir}/{data_type}_{uid}.npy", demeaned_imgs[i])
    ...
    for i in range(len(dynamic_imgs) - 1):
        ...
        for j in range(i + 1, len(dynamic_imgs)):
            ...
            np.save(f"{delta_rgb_img_dir}/{data_type}_{cid}_{year_1}_{year_2}.npy", delta_img)
...
pth = f"{sat_img_dir}/LS/LS_rgb_delta_img_stats.pkl"

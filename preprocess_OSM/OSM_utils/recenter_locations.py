...
def recenter_location(cluster_info, nearest_geom, dist):
    if nearest_geom is None:
      ...
      closest_point = nearest_points(intersection, cluster_info.geometry)[0] # select the first, as this is the closes point on the polygon
...
def get_nearest_geom(cluster_info, response_df, extreme = False):
    ...
    if cluster_info.rural == 1:
      if extreme: radius = 10000
      else: radius = 5000
    else:
      if extreme: radius = 5000
      else: radius = 2000

    # Use a spatial index to find the nearest point
    sindex = proj_geom.sindex
    nearest_idx, dist = sindex.nearest(proj_point.values[0][0], max_distance = radius, return_all = False, return_distance = True)
    if len(dist) == 0:
      nearest_geom = None
      dist = np.nan
    else:
...
def download_OSM_data(bpolys, fltr, client):
    pre = datetime.now()
